...
export default function EmploiTempsPage() {
  ...
  const supprimerET = async (id) => {
    if (!window.confirm('Supprimer cet emploi du temps ? Cette action est irréversible.')) return;
    try {
      await api.del('/emploi-temps?id=' + id);
      success('Emploi du temps supprimé');
      setCurrentET(null);
      await loadEmplois(selectedClasse);
    } catch(e) { error(e.message); }
  };
  ...
  const supprimerCreneau = async (creneauId) => {
    ...
    try {
      await api.del('/creneaux?id=' + creneauId);
      success('Créneau supprimé');
      await loadEmplois(selectedClasse);
    } catch(e) { error(e.message); }
  };

  // Ajouter créneau à un ET existant
  const [showAddCreneau, setShowAddCreneau] = useState(false);
  const [newCr, setNewCr] = useState({ jour:'lundi', heure_debut:'08:00', heure_fin:'10:00', id_matiere:'', id_enseignant:'', id_salle:'' });
  const setNewCrField = (k, v) => setNewCr(prev => Object.assign({}, prev, { [k]: v }));

  const ajouterCreneauExistant = async () => {
    if (!newCr.id_matiere || !newCr.id_enseignant || !newCr.id_salle) { error('Matière, enseignant et salle requis'); return; }
    try {
      await api.post('/creneaux', { ...newCr, id_emploi_temps: currentET.id });
                ...
                {JOURS.map(j => {
                  ...
                  return (
                    <td key={j} style={{ padding:4, border:'1px solid #f1f5f9', verticalAlign:'top', height:50 }}>
                      {cr && (
                        <div style={{ background: matColor(cr.id_matiere), borderRadius:6, padding:'5px 7px', color:'#fff', fontSize:'0.7rem', height:'100%', boxSizing:'border-box', position:'relative' }}>
                          ...
                          {cr.qr_token && <div style={{ fontSize:'0.62rem', background:'rgba(255,255,255,0.25)', borderRadius:3, padding:'1px 4px', marginTop:2 }}>📱 QR</div>}
                          {user.role === 'admin' && (
                            <button onClick={() => supprimerCreneau(cr.id)}
                              style={{ position:'absolute', top:2, right:2, background:'rgba(0,0,0,0.3)', border:'none', borderRadius:4, color:'#fff', cursor:'pointer', fontSize:'0.65rem', padding:'1px 4px', lineHeight:1.4 }}
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ background:'#fff', borderRadius:12, padding:20, boxShadow:'0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              ...
              {user.role === 'admin' && currentET.statut_publication !== 'publie' && (
                <button onClick={() => publier(currentET.id)} disabled={publishing}
                  ...
                  {publishing ? '⏳...' : '📢 Publier & QR'}
                </button>
              )}
              {user.role === 'admin' && (
                <>
                  <button onClick={() => setShowAddCreneau(!showAddCreneau)}
                    className="btn btn-primary btn-sm">
                    {showAddCreneau ? '✕ Fermer' : '➕ Ajouter créneau'}
                  </button>
                  <button onClick={() => supprimerET(currentET.id)}
          ...
          {/* Formulaire ajout créneau à ET existant */}
          {showAddCreneau && user.role === 'admin' && (
            <div style={{ background:'#F8FAFC', border:'2px dashed var(--primary)', borderRadius:10, padding:16, marginBottom:16 }}>
              <p className="section-title" style={{ marginBottom:12 }}>➕ Ajouter un créneau à cet emploi du temps</p>
              <div className="form-grid form-grid-3" style={{ gap:10, marginBottom:10 }}>
                <div>
                  <label className="label">Jour</label>
                  <select value={newCr.jour} onChange={e => setNewCrField('jour', e.target.value)} className="input">
                    {JOURS.map(j => <option key={j} value={j}>{j}</option>)}
                  </select>
                </div>
                <div>
                  <label className="label">Début</label>
                  <input type="time" value={newCr.heure_debut} onChange={e => setNewCrField('heure_debut', e.target.value)} className="input" />
                </div>
                <div>
                  <label className="label">Fin</label>
                  <input type="time" value={newCr.heure_fin} onChange={e => setNewCrField('heure_fin', e.target.value)} className="input" />
                </div>
                <div>
                  <label className="label">Matière</label>
                  <select value={newCr.id_matiere} onChange={e => setNewCrField('id_matiere', e.target.value)} className="input">
                    <option value="">-- Matière --</option>
                    {matieres.map(m => <option key={m.id} value={m.id}>{m.libelle}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={newCr.id_enseignant} onChange={e => setNewCrField('id_enseignant', e.target.value)} className="input">
                    <option value="">-- Enseignant --</option>
                    {enseignants.map(e => <option key={e.id} value={e.id}>{e.nom} {e.prenom}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <select value={newCr.id_salle} onChange={e => setNewCrField('id_salle', e.target.value)} className="input">
                    <option value="">-- Salle --</option>
              ...
              <div className="actions">
                <button onClick={ajouterCreneauExistant} className="btn btn-success btn-sm">✓ Ajouter</button>
                <button onClick={() => setShowAddCreneau(false)} className="btn btn-ghost btn-sm">Annuler</button>
              </div>
            </div>
          )}
